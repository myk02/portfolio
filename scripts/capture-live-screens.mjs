...
/** name → url (+ optional scrollTo in px before the shot) */
const TARGETS = [
  {
    slug: "kenyatrace",
    ...
    screens: [
      ...
      { name: "discover", path: "/discover", viewports: ["mobile", "tablet", "desktop"] },
      { name: "plan", path: "/plan", viewports: ["mobile", "tablet", "desktop"] },
      { name: "trips", path: "/trips", viewports: ["mobile", "tablet", "desktop"] },
      { name: "stays", path: "/discover/stays", viewports: ["mobile", "tablet", "desktop"] },
    ],
  },
  {
    slug: "gigi-energy",
    ...
    screens: [
      { name: "home", path: "/", viewports: ["mobile", "tablet", "desktop"] },
      { name: "flavours", path: "/flavours", viewports: ["mobile", "tablet", "desktop"] },
      { name: "events", path: "/events", viewports: ["mobile", "tablet", "desktop"] },
      { name: "home-scroll", path: "/", scroll: 1100, viewports: ["mobile", "tablet", "desktop"] },
    ],
  },
];
...
for (const target of TARGETS) {
  ...
  for (const screen of target.screens) {
    for (const vp of screen.viewports) {
      const viewport = vp === "mobile" ? MOBILE : vp === "tablet" ? TABLET : DESKTOP;
      ...
      try {
        ...
        console.log(`  ok  ${target.slug}/${screen.name}-${vp}.jpg  (${kb} KB)`);
      } catch (err) {
        console.error(`  FAIL ${target.slug}/${screen.name}-${vp}: ${err.message}`);
      } finally {
        await ctx.close();
      }
    }
