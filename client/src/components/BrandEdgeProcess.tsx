    ...
    <motion.div style={{ opacity }} className="border-t border-secondary/10 pt-4 pb-3">
      <div className="grid grid-cols-12 gap-3 lg:gap-5">
        <div className="col-span-1">
          <span className="font-mono text-[12px] tracking-wider text-accent">{step.number}</span>
        </div>
        <div className="col-span-3 lg:col-span-3">
            <h3 className="font-display font-bold text-secondary" style={{ fontSize: "clamp(1rem, 1.8vw, 1.3rem)" }}>
              {step.title}
            </h3>
            <span className="font-mono text-[13px] tracking-wider text-secondary/30">{step.duration}</span>
        </div>
        <div className="col-span-5 lg:col-span-5">
          <p className="text-secondary/60 leading-relaxed text-[17px]">{step.description}</p>
        </div>
        <div className="col-span-3 lg:col-span-3">
          <span className="font-mono text-[13px] tracking-[0.2em] uppercase text-secondary/40">Deliverables</span>
          <ul className="mt-2 space-y-1.5">
            {step.deliverables.map((d) => (
              <li key={d} className="flex items-center gap-2 text-[17px] text-secondary/50">
      ...
      <div className="container">
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-3 mb-6">
          <div>
            ...
            <h2 className="heading-serif font-bold text-secondary" style={{ fontSize: "clamp(1.5rem, 3vw, 2.3rem)" }}>
              Three steps.
              <span className="italic font-light text-accent block text-sm"> No fluff.</span>
            </h2>
          </div>
          <p className="text-[17px] text-secondary/50 max-w-xs leading-relaxed lg:text-right">
